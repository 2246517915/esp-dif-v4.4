...
void bt_mesh_model_recv(struct bt_mesh_net_rx *rx, struct net_buf_simple *buf)
{
	...
	*/
	...
	/*Catch filepacker*/
	BT_WARN("src:0x%04x, dst:0x%04x, SEQ:0x%08x,IVI:0x%08x, TTL:%u, Len: %d,  data: %s",
           rx->ctx.addr, rx->ctx.recv_dst, rx->seq,BLE_MESH_NET_IVI_RX(rx), rx->ctx.recv_ttl, buf->len,bt_hex(buf->data, buf->len));
