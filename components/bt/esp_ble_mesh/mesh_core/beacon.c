                           ...
                           struct net_buf_simple *buf)
{
    ...
    BT_DBG("net_idx 0x%03x iv_index 0x%08x flags 0x%02x",
...
static void secure_beacon_recv(struct net_buf_simple *buf)
{
    ...
    cache_add(data, sub);
    BT_INFO("flags 0x%02x id %s iv_index 0x%08x",
           flags, bt_hex(net_id, 8), iv_index);
    ...
    BT_INFO("flags = %d, BLE_MESH_IV_UPDATE(flags) = %d\n",flags,BLE_MESH_IV_UPDATE(flags) );
    iv_change = bt_mesh_net_iv_update(iv_index,true);// BLE_MESH_IV_UPDATE(flags));
