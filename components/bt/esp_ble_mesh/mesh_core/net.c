...
#define IV_UPDATE_SEQ_LIMIT 0xFFFFFF/2
...
void bt_mesh_rpl_seq_clear(void)
{
	int i;
...
void bt_mesh_rpl_query(void)
{
	...
	for (i = 0; i < ARRAY_SIZE(bt_mesh.rpl); i++) {
		struct bt_mesh_rpl *rpl = &bt_mesh.rpl[i];
		if (rpl->src) {
			BT_INFO("RPL : src[%04x],seq[%08x],old_iv[%d]",rpl->src,rpl->seq,rpl->old_iv);
		}
	}
}
...
uint32_t bt_mesh_next_seq(void)
{
    ...
    BT_WARN("Current Iv_index : %08x , Ble Mesh Seq : %08x",bt_mesh.iv_index,bt_mesh.seq);
    if( bt_mesh.seq > IV_UPDATE_SEQ_LIMIT && bt_mesh_primary_subnet_exist()){
        bt_mesh_iv_update();
    }
                      ...
                      enum bt_mesh_net_if net_if)
{
	...
	/*Catch filepacker Comment out the*/
