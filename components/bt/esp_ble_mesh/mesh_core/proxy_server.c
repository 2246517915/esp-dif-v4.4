...
// }
static int bt_proxy_status = 0;
static void bt_proxy_server_con_sta_set(int status)
{
    bt_proxy_status = status;
}

int bt_proxy_server_ccon_sta_get(void)
{
    return bt_proxy_status;
}
...
//     }
    bt_proxy_server_con_sta_set(true);
#endif
    net_buf_simple_reset(&client->buf);
}
...
//             }
// #endif
            bt_proxy_server_con_sta_set(false);
...
const struct  bt_mesh_adv_data node_id_ad[] = {
    BLE_MESH_ADV_DATA_BYTES(BLE_MESH_DATA_FLAGS, (BLE_MESH_AD_GENERAL | BLE_MESH_AD_NO_BREDR)),
...
const struct  bt_mesh_adv_data net_id_ad[] = {
    BLE_MESH_ADV_DATA_BYTES(BLE_MESH_DATA_FLAGS, (BLE_MESH_AD_GENERAL | BLE_MESH_AD_NO_BREDR)),
    BLE_MESH_ADV_DATA_BYTES(BLE_MESH_DATA_UUID16_ALL, 0x28, 0x18),
...
// void bt_mesh_adv_data_set_net(struct bt_mesh_adv_data *data,uint8_t len)
// {
//     esp_log_buffer_hex("data",&data[0],sizeof(data[0]));
//     if(!data){
//         BT_WARN("bt_mesh_adv_data_set_node data (is null");
//         return;
//     }
//     for(int i = 0;i < len;i++){
//         memcpy(&net_id_ad[1+i],&data[i],sizeof(data[i]));
//     }
    
// }

// void bt_mesh_adv_data_set_node(struct bt_mesh_adv_data *data,uint8_t len)
// {
//     esp_log_buffer_hex("data",&data[0],sizeof(data[0]));
//     if(!data){
//         BT_WARN("bt_mesh_adv_data_set_node data (is null");
...
//     for(int i = 0;i < len;i++){
//         memcpy(&node_id_ad[1+i],&data[i],sizeof(data[i]));
//     }
// }
/**********FangRong****************/
static struct bt_mesh_adv_data  *padv_rsp = NULL;
static int adv_rsp_num = 0;
static int adv_rsp_mode = 0;
void ble_mesh_proxy_adv_rsp_data(uint8_t mode,struct bt_mesh_adv_data *proxy_sd,uint8_t type_num)
{
    size_t len = 0;

    adv_rsp_mode = mode;
    if(!proxy_sd || !adv_rsp_mode) return;

    padv_rsp = (struct bt_mesh_adv_data *)malloc(sizeof(struct bt_mesh_adv_data)*type_num);
    for(int i = 0;i < type_num;i++){
        //padv_rsp[i] = (struct bt_mesh_adv_data *)malloc(sizeof(struct bt_mesh_adv_data));
        //padv_rsp[i]->data = (uint8_t *)malloc(proxy_sd[i].data_len);
        padv_rsp[i] = proxy_sd[i];
        len++;
    }
...
static int node_id_adv(struct bt_mesh_subnet *sub)
{
    ...
    err = bt_le_adv_start(&fast_adv_param, node_id_ad,
                          ARRAY_SIZE(node_id_ad), (adv_rsp_mode)?padv_rsp:(&proxy_sd), (adv_rsp_mode)?adv_rsp_num:proxy_sd_len);
...
static int net_id_adv(struct bt_mesh_subnet *sub)
{
    ...
    err = bt_le_adv_start(&slow_adv_param, net_id_ad,
                          ARRAY_SIZE(net_id_ad), (adv_rsp_mode)?padv_rsp:(&proxy_sd), (adv_rsp_mode)?adv_rsp_num:proxy_sd_len);
